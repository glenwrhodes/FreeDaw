...
| Transport             | Play/stop/record/loop toggle, BPM, time signature, loop in/out range with draggable ruler handles, metronome toggle, count-in with configurable length in `src/ui/transport/TransportBar.cpp` and `src/ui/timeline/TimeRuler.cpp` | ~~No loop in/out range UI, no count-in/pre-roll~~, no punch in/out **DONE (loop/metro/count-in)**                         |
...
| MIDI note editing     | Note draw/select/move/resize, quantize, velocity lane in `src/ui/pianoroll/NoteGrid.cpp` and `VelocityLane.cpp`; CC lane editor with point editing in `src/ui/pianoroll/CcLane.cpp`; multi-channel piano roll overlay with per-channel colors, visibility toggles, and renaming in `PianoRollEditor.cpp` | ~~No CC lanes~~, ~~no MIDI channel chooser~~, no pitch-bend/aftertouch editing                                                    |
...
2. **MIDI CC + channel fundamentals** -- **DONE**
  - ~~CC lane editor (minimum CC1, CC7, CC10, CC11 + arbitrary CC picker).~~ **DONE**
  - ~~Per-clip MIDI channel assignment (1-16) with multi-channel piano roll overlay.~~ **DONE**
3. **Recording ergonomics baseline** -- **DONE**
...
- ~~User can set loop start/end visually and record repeated passes against that loop.~~ **DONE**
- ~~User can enable metronome and optional count-in and hear click in recording workflow.~~ **DONE**
...
- ~~User can select MIDI channel behavior per track or clip and get predictable playback/output.~~ **DONE**
...
3. ~~MIDI CC lanes and channel assignment.~~ **DONE**
4. ~~Loop in/out UI, metronome, and count-in.~~ **DONE**
